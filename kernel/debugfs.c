...
static int tbv_debugfs_summary_show(struct seq_file *s, void *unused)
{
	...
	for (i = 0; i < TBV_DV_WRITE_LOCAL_BUCKETS; i++) {
		...
		seq_printf(s, "dv_write_submit_mr_bucket_%u_ns: %lld\n", i,
			   atomic64_read(
				   &state->dv_write_submit_mr_bucket_ns[i]));
		seq_printf(s, "dv_write_enqueue_mr_bucket_%u_ns: %lld\n", i,
			   atomic64_read(
				   &state->dv_write_enqueue_mr_bucket_ns[i]));
		seq_printf(s, "dv_write_drain_mr_bucket_%u_ns: %lld\n", i,
			   atomic64_read(
				   &state->dv_write_drain_mr_bucket_ns[i]));
	...
	seq_printf(s, "data_rx_ack_req_miss_current_active: %lld\n",
		   atomic64_read(&state->data_rx_ack_req_miss_current_active));
	seq_printf(s, "data_rx_ack_req_miss_current_reorder: %lld\n",
		   atomic64_read(&state->data_rx_ack_req_miss_current_reorder));
	seq_printf(s, "data_rx_ack_req_miss_current_idle: %lld\n",
		   atomic64_read(&state->data_rx_ack_req_miss_current_idle));
	...
	seq_printf(s, "data_rx_write_gap_rnr: %lld\n",
		   atomic64_read(&state->data_rx_write_gap_rnr));
