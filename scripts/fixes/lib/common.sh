#!/usr/bin/env bash
# Shared helpers for the fixes automation scripts.
#
# These functions are sourced by the numbered per-fix scripts in this directory.
# They wrap `gh` and `git` so each fix can, idempotently:
...
#
# This mirrors the (now archived) roadmap toolkit, but files work for the driver
# analysis findings in docs/FINDINGS.md instead of roadmap steps: branches are
# cut under the "fix" prefix and the seed spec is dropped at
# docs/fixes/steps/<NN>-<slug>.md.
#
...
# Branch every fix branch and PR is cut from. Auto-detected from the repo's
# default branch when left empty.
BASE_BRANCH="${BASE_BRANCH:-}"

# Prefix applied to every fix branch name, e.g. fix/01-reliability-dedup-window.
...
tag_copilot_on_pr() {
  ...
  local body
  body="$(printf '%s please implement this fix.\n\nTracking issue: #%s\n\n%s' \
...
# ----------------------------------------------------------------------------
# The one entry point each fix script calls.
# ----------------------------------------------------------------------------

# run_fix_step keeps the per-fix scripts tiny: they just define the spec
# variables below and call this. Required variables:
#   STEP_NUM    e.g. 01
#   STEP_SLUG   e.g. reliability-dedup-window
...
run_fix_step() {
  ...
  local seed_rel="docs/fixes/steps/$STEP_NUM-$STEP_SLUG.md"
  ...
  create_branch_with_seed "$branch" "$seed_rel" "$seed" \
    "docs(fixes): seed fix $STEP_NUM ($STEP_SLUG)"
  ...
  echo
  ok "fix $STEP_NUM done — issue #$issue, branch $branch, PR #$pr"
}
